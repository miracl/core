...
// tau,gamma1,gamma2,K,L,eta,lg(2*eta+1),omega,lambda
...
// secret key of size 64+TRSIZE+DEGREE*(K*D+L*LG2ETA1+K*LG2ETA1)/8
...
// CRH(rho,t1)
static void CRH1(const int *params,byte H[TRSIZE],byte rho[32],sign16 t1[]) /*** ML_DSA ***/
{
    int i;
    int ptr,bts;
    int ck=params[3];
...
static int UsePartialHint(const int *params,sign8 r[],byte h[],int hptr,int i,sign32 w[])
{
    ...
    for (m=0;m<DL_DEGREE;m++)
    {
        ...
        if (m==h[hptr] && hptr<h[omega+i])  // if h[] not in ascending order this will fail and sig will not verify
        {
            hptr++;
...
static int signature(const int *params,bool ph,byte *rn,octet *sk,octet *CTX,octet *M,octet *sig)
{
    int i,k,nh,fk,row,j;
    bool badone;
    byte rho[32];
...
static bool verify(const int *params,bool ph,octet *pk,octet *CTX,octet *M,octet *sig)
{
    int i,row,j,m,hints;
    byte rho[32];
    byte mu[64];
...
int core::DLTHM_signature_2(bool ph,byte *rn,octet *sk,octet *CTX,octet *M,octet *sig)
{
    return signature(PARAMS_2,ph,rn,sk,CTX,M,sig);
}

bool core::DLTHM_verify_2(bool ph,octet *pk,octet *CTX,octet *M,octet *sig)
{
...
int core::DLTHM_signature_3(bool ph,byte *rn,octet *sk,octet *CTX,octet *M,octet *sig)
{
    return signature(PARAMS_3,ph,rn,sk,CTX,M,sig);
}

bool core::DLTHM_verify_3(bool ph,octet *pk,octet *CTX,octet *M,octet *sig)
{
...
int core::DLTHM_signature_5(bool ph,byte *rn,octet *sk,octet *CTX,octet *M,octet *sig)
{
    return signature(PARAMS_5,ph,rn,sk,CTX,M,sig);
}

bool core::DLTHM_verify_5(bool ph,octet *pk,octet *CTX,octet *M,octet *sig)
{
