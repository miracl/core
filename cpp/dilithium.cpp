...
const sign32 inv = 0x3ffe;
const sign32 invpr = 0x7b60bc;
...
// pack public key
static void pack_pk(byte pk[],byte rho[32],sign16 t1[])
{
    int ptr,bts,i,n=0;
    for (i=0;i<32;i++)
        pk[i]=rho[i];
...
// unpack public key
static void unpack_pk(byte rho[32],sign16 t1[],byte pk[])
{
    int ptr,bts,i;
    for (i=0;i<32;i++)
        rho[i]=pk[i];
...
static void pack_sk(byte sk[],byte rho[32],byte bK[32],byte tr[32],sign8 s1[],sign8 s2[],sign16 t0[])
{
    int ptr,bts,i,n=32;
    for (i=0;i<32;i++)
        sk[i]=rho[i];
...
static void sample_Sn(byte rhod[64],sign8 s[],int n)
{
    int m,ptr,bts;
    sha3 sh;
    byte buff[272];
...
static void sample_Y(int k,byte rhod[64],sign32 y[])
{
...
static sign16 power2round(sign32 *r0)
{
    int d=(1<<D);
    sign32 r1;
    r1=(*r0+d/2-1)>>D;
...
static sign32 decompose_lo(sign32 a) {
  sign32 a0,a1;
  a1  = (a + 127) >> 7;
  a1  = (a1*1025 + (1 << 21)) >> 22;
  a1 &= 15;
...
// ALPHA = (Q-1)/16
static sign8 decompose_hi(sign32 a) {
  sign32 a1;
  a1  = (a + 127) >> 7;
  a1  = (a1*1025 + (1 << 21)) >> 22;
  a1 &= 15;
...
static int MakePartialHint(byte h[],int hptr,sign32 z[],sign32 r[])
{
    int m;
    sign8 a0,a1;
    sign32 rz,t;
...
int core::DLTHM_signature(octet *sk,octet *M,octet *sig)
{
    ...
    sign32 w[DEGREE];  // work space 1024 bytes
    sign32 r[DEGREE];  // work space 1024 bytes total= 21853
...
// Calculate Ay 
        for (i=0;i<K;i++)
        {
            ...
            for (j=0;j<L;j++)
            {
                ...
                ExpandAij(rho,c,i,j);  // use c for Aij
                poly_mul(w,w,c);
                poly_add(r,r,w);
                //poly_soft_reduce(r);  // be lazy
            }
...
// Calculate Az-ct1.2^d
        ...
        hints=UsePartialHint(&w1d[row],hint,hints,i,w);
