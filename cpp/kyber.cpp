...
// NOTE - should make sure Public key is properly constructed if not sure of its provenance
// See section 7.2 of https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf
// Public keys generated by this code will be OK, so checking not required if this code used.
static void KYBER_CCA_encrypt(const int *params,byte *randbytes32,octet *pk,octet *ss,octet *ct)
{
    ...
    for (i=0;i<32;i++)
        SHA3_process(&sh,randbytes32[i]);      // seed
...
{ 
    ...
    SHA3_init(&sh,SHA3_HASH512);               // Kb,r = G(m|h)
    for (i=0;i<32;i++)
        SHA3_process(&sh,m[i]);
    for (i=0;i<32;i++)
        SHA3_process(&sh,h[i]);
