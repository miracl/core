...
static void ZZZ::mpin_hash(int sha, FP16 *f, ECP *P, octet *w)
{
    ...
    FP_redc(x, &(f->b.a.a.b));
    BIG_toBytes(&T.val[9 * MODBYTES_XXX], x);

    FP_redc(x, &(f->b.a.b.a));
    BIG_toBytes(&T.val[10 * MODBYTES_XXX], x);
    FP_redc(x, &(f->b.a.b.b));
    BIG_toBytes(&T.val[11 * MODBYTES_XXX], x);

    FP_redc(x, &(f->b.b.a.a));
    BIG_toBytes(&T.val[12 * MODBYTES_XXX], x);
    FP_redc(x, &(f->b.b.a.b));
    BIG_toBytes(&T.val[13 * MODBYTES_XXX], x);

    FP_redc(x, &(f->b.b.b.a));
    BIG_toBytes(&T.val[14 * MODBYTES_XXX], x);
    FP_redc(x, &(f->b.b.b.b));
    BIG_toBytes(&T.val[15 * MODBYTES_XXX], x);

    ECP_get(x, y, P);
    BIG_toBytes(&T.val[16 * MODBYTES_XXX], x);
    BIG_toBytes(&T.val[17 * MODBYTES_XXX], y);
